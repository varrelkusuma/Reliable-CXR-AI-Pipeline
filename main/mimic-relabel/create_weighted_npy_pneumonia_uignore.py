...
def get_label(idx):
    """
    Standard label fetch.
    Since MIMIC rows were dropped, and other datasets are clean,
    we expect standard 0 or 1.
    """
    try:
        sample = dmerge[idx]
        lab = sample["lab"][pathology_idx]
        
        # Standard fallback: NaN -> 0
        if np.isnan(lab):
            return 0 
...
print(f"Extracting labels (Dataset Size: {len(dmerge)})...")
# Note: The len(dmerge) should be ~14k smaller than your previous run
...
print(f"Dataset Stats (U-Ignore): Neg={n_neg}, Pos={n_pos}")
...
np.save("pneumonia_train_weights_ignore.npy", sample_weights)
np.save("pneumonia_pos_weight_uignore.npy", np.array([pos_weight_value]))

print(f"Saved 'pneumonia_train_weights_ignore.npy' (len={len(sample_weights)})")
print(f"Saved 'pneumonia_pos_weight_uignore.npy' (value={pos_weight_value:.4f})")