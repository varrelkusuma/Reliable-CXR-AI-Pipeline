...
if "Lung Lesion" not in d_nih_train.pathologies:
    d_nih_train.pathologies.append("Lung Lesion")

mass_idx = d_nih_train.pathologies.index("Mass")
nodule_idx = d_nih_train.pathologies.index("Nodule")

# Original labels shape: (N, 14)
old_labels = d_nih_train.labels
N, old_len = old_labels.shape
new_len = len(d_nih_train.pathologies)
...
lunglesion_idx = d_nih_train.pathologies.index("Lung Lesion")
...
d_nih_train.labels = new_labels

"""---- Create Lung Lesion Label for NIH Dataset (Selection) ---"""
# create NIH label (lung lesion) if not present
if "Lung Lesion" not in d_nih_selection.pathologies:
    d_nih_selection.pathologies.append("Lung Lesion")

mass_idx = d_nih_selection.pathologies.index("Mass")
nodule_idx = d_nih_selection.pathologies.index("Nodule")

# Original labels shape: (N, 14)
old_labels = d_nih_selection.labels
N, old_len = old_labels.shape
new_len = len(d_nih_selection.pathologies)
...
# Fill Lung Lesion column as OR(Mass, Nodule)
lunglesion_idx = d_nih_selection.pathologies.index("Lung Lesion")
new_labels[:, lunglesion_idx] = np.logical_or(
    old_labels[:, mass_idx] == 1,
    old_labels[:, nodule_idx] == 1
).astype(int)
...
# Replace dataset labels
d_nih_selection.labels = new_labels
...
for epoch in range(num_epochs):
    ...
    }, f"/home/varrel/image_model_dict/lesion_model_epoch{epoch+1}.pth")
    ...
    if val_f1 > best_val_f1:
        torch.save(model.state_dict(), "/home/varrel/image_model_dict/lesion_best_model.pth")
        best_val_f1 = val_f1
        epochs_no_improve = 0
    else:
        epochs_no_improve += 1
...
# ---- Save history JSON ----
with open("/home/varrel/image_model_dict/lesion_training_history.json", "w") as f:
    json.dump(history, f)
