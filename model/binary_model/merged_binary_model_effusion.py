...
d_mimic_selection.pathologies = [
    "Pleural Effusion" if p == "Effusion" else p
    for p in d_mimic_selection.pathologies
]
d_chex_train.pathologies = [
    "Pleural Effusion" if p == "Effusion" else p
    for p in d_chex_train.pathologies
]
d_chex_selection.pathologies = [
    "Pleural Effusion" if p == "Effusion" else p
    for p in d_chex_selection.pathologies
]
d_nih_train.pathologies = [
    "Pleural Effusion" if p == "Effusion" else p
    for p in d_nih_train.pathologies
]
d_nih_selection.pathologies = [
    "Pleural Effusion" if p == "Effusion" else p
    for p in d_nih_selection.pathologies
]
...
sample_weights = np.load("/home/varrel/GitHub/CXR-Analysis/model/effusion_train_weights.npy")
pos_weight_value = np.load("/home/varrel/GitHub/CXR-Analysis/model/effusion_pos_weight.npy")[0]
...
for epoch in range(num_epochs):
    ...
    }, f"/home/varrel/image_model_dict/effusion_model_epoch{epoch+1}.pth")
    ...
    if val_f1 > best_val_f1:
        torch.save(model.state_dict(), "/home/varrel/image_model_dict/effusion_best_model.pth")
        best_val_f1 = val_f1
        epochs_no_improve = 0
    else:
        epochs_no_improve += 1
...
# ---- Save history JSON ----
with open("/home/varrel/image_model_dict/effusion_training_history.json", "w") as f:
    json.dump(history, f)
