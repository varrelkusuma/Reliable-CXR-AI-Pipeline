...
def safe_collate(batch):
    ...
    for b in batch:
        ...
        img = torch.as_tensor(b["img"], dtype=torch.float32)
        lab = torch.as_tensor(b["lab"])

        # 1. Handle Channel Dimensions (Ensure 3-channel)
        if img.ndim == 3 and img.shape[0] == 1: 
            img = img.repeat(3, 1, 1)
        elif img.ndim == 2: 
            img = img.unsqueeze(0).repeat(3, 1, 1)

        # 2. Fix Image Size (The culprit)
        # Check if height or width is not 512
        if img.shape[-2:] != target_size:
            # Interpolate expects 4D (Batch, Channel, H, W)
            img = img.unsqueeze(0) 
...
d_chex_raw = xrv.datasets.CheX_Dataset(
    imgpath="/home/varrel/imaging/all_images/CheX_dataset/images_all/CheXpert-v1.0/preprocess8",
...
d_mimic_raw = xrv.datasets.MIMIC_Dataset(
    imgpath="/home/varrel/imaging/all_images/MIMIC_dataset/images_all/preprocess8",
...
pc_png_selection = PNGFolderDataset(
    img_dir="/home/varrel/imaging/all_images/PC_dataset/images_all/preprocess8",
...
vindr_png_selection = PNGFolderDataset(
    img_dir="/home/varrel/imaging/all_images/VinDr_dataset/images_all/preprocess8",
...
openi_png_selection = PNGFolderDataset(
    img_dir="/home/varrel/imaging/all_images/Openi_dataset/images_all/preprocess8",
...
dmerge_selection = xrv.datasets.MergeDataset([
    d_chex_selection, d_mimic_selection, 
...
model_paths = {
    "SwinB": "/home/varrel/image_model_dict/final/fixed_swinb_fracture_model_epoch1.pth",
}
...
# Save results
save_path = "/home/varrel/image_model_dict/final/fixed_fracture_inference.json"
