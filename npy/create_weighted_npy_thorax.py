...
dmerge = xrv.datasets.MergeDataset([
    ...
    d_mimic_train, pc_png_train, vindr_png_train, siim_png_train
])
...
pathology_idx = dmerge.pathologies.index("Pneumothorax")
...
np.save("pneumothorax_train_weights.npy", sample_weights)
print(f"Weighted sampler saved as pneumothorax_train_weights.npy (len={len(sample_weights)})")